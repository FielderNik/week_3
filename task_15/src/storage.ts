...
export type ArtifactFileDraft = {
  path: string;
  content: string;
};

export type PersistedArtifactFile = {
  path: string;
};
...
export async function loadInvariants() {
  return loadMemoryDocument(invariantsFileName);
}

export async function persistInvariants(content: string) {
  return persistMemoryDocument(invariantsFileName, content);
}
...
export async function persistArtifactFiles(
  parentChatId: string,
  artifactFiles: ArtifactFileDraft[],
  ...
  const artifactRoot = await directory.getDirectoryHandle(artifactsDirectoryName, { create: true });
  const chatDirectoryName = sanitizePathSegment(parentChatId) || "chat";
  const chatArtifactsDirectory = await artifactRoot.getDirectoryHandle(chatDirectoryName, { create: true });
  const persistedFiles: PersistedArtifactFile[] = [];

  for (const artifactFile of artifactFiles) {
    const pathSegments = toSafeArtifactPathSegments(artifactFile.path);
    const fileName = pathSegments[pathSegments.length - 1];
    let targetDirectory = chatArtifactsDirectory;
    ...
    const writable = await file.createWritable();
    await writable.write(artifactFile.content);
    await writable.close();

    persistedFiles.push({
      path: `${dialogsDirectoryName}/${artifactsDirectoryName}/${chatDirectoryName}/${pathSegments.join("/")}`,
    });
  }
...
async function readSavedChatsFromDirectory(
  ...
  for await (const entry of toFileStorageDirectory(directory).values()) {
    ...
    if (
      entry.name === projectMemoryFileName ||
      entry.name === longTermMemoryFileName ||
      entry.name === userProfileFileName ||
      entry.name === invariantsFileName
    ) {
      continue;
    }
...
function toSafeArtifactPathSegments(path: string) {
  const segments = path
    .replaceAll("\\", "/")
    .split("/")
  ...
  if (segments.length === 0 || path.split(/[\\/]/).some((segment) => segment.trim() === "..")) {
    throw new Error(`Некорректный путь артефакта: ${path}`);
  }
...
function isSavedChat(value: unknown): value is SavedChat {
  ...
  return (
    ...
    (candidate.taskState === undefined || isTaskState(candidate.taskState)) &&
    (candidate.agent === undefined || isAgentChatMetadata(candidate.agent)) &&
...
function isTaskState(value: unknown): value is TaskState {
  ...
  return (
    ...
    (candidate.askBeforeStageTransition === undefined || typeof candidate.askBeforeStageTransition === "boolean") &&
    Array.isArray(candidate.steps) &&
    candidate.steps.every(isTaskStep) &&
    typeof candidate.updatedAt === "string"
  );
}
...
function isTaskStep(value: unknown): value is TaskStep {
  ...
  return (
    ...
    (candidate.phase === undefined ||
      candidate.phase === "research" ||
      candidate.phase === "execution" ||
      candidate.phase === "validation") &&
    typeof candidate.title === "string" &&
    isTaskStepStatus(candidate.status) &&
    (candidate.agentDialogId === null || typeof candidate.agentDialogId === "string") &&
    (candidate.artifactPaths === undefined ||
      (Array.isArray(candidate.artifactPaths) && candidate.artifactPaths.every((path) => typeof path === "string"))) &&
    typeof candidate.resultSummary === "string" &&
    typeof candidate.updatedAt === "string"
  );
}

function isAgentChatMetadata(value: unknown): value is AgentChatMetadata {
  if (!value || typeof value !== "object") {
    return false;
  }

  const candidate = value as Partial<AgentChatMetadata>;
  ...
  return (
    candidate.role === "step_agent" &&
    typeof candidate.parentChatId === "string" &&
    typeof candidate.stepId === "string"
  );
}

function isTaskPhase(value: unknown): value is TaskPhase {
  return value === "research" || value === "execution" || value === "validation" || value === "done";
}

function isTaskStepStatus(value: unknown): value is TaskStepStatus {
  return value === "pending" || value === "in_progress" || value === "blocked" || value === "done" || value === "failed";
}
