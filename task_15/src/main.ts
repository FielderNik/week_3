...
import {
  advanceTaskPhase,
  canRunCurrentStep,
  createInvalidTransitionAssistantMessage,
  getCurrentTaskStep,
  getNextTaskPhase,
...
import type { PipelinePhase } from "./taskLifecycle";
import {
  appendSavedChatLog,
...
type MemoryKind = "project" | "longTerm" | "invariants";
type InvariantBlockDetails = {
  violatedInvariant: string;
  reason: string;
  userMessage: string;
  summary: string;
};
type AgentStructuredStatus = {
  status: "done" | "blocked_by_invariant" | "blocked" | "failed";
  details: InvariantBlockDetails;
};
...
let invariants: MemoryDocument = { ...emptyMemoryDocument };
  ...
  <main class="app-shell">
    ...
    <aside class="settings-panel" aria-label="Chat settings">
      ...
      <section class="memory-panel" aria-label="Память ассистента">
        ...
        <button class="memory-button" data-action="open-invariants" type="button">
          <strong>Инварианты</strong>
          <span data-invariants-status>пусто</span>
        </button>
...
const invariantsStatusElement = appRoot.querySelector<HTMLElement>("[data-invariants-status]")!;
...
function normalizeTaskState(taskState: TaskState): TaskState {
  ...
  const currentStepMatchesPhase = steps.some((step) => step.id === taskState.currentStepId && step.phase === currentPhase);

  return {
    ...taskState,
    phase,
    currentStepId: currentStepMatchesPhase ? taskState.currentStepId : currentStep?.id || null,
...
function getCurrentStep(taskState: TaskState = ensureCurrentTaskState()) {
  return getCurrentTaskStep(taskState);
}
...
function createStepAgentPrompt(parentTitle: string, parentTaskBrief: string, taskState: TaskState, step: TaskStep) {
  ...
  return [
    ...
    "- первой строкой обязательно укажи `AGENT_STATUS: done`, `AGENT_STATUS: blocked_by_invariant`, `AGENT_STATUS: blocked` или `AGENT_STATUS: failed`",
    "- если запрос нарушает инварианты, используй только `AGENT_STATUS: blocked_by_invariant`, не создавай артефакты и не предлагай нарушающее решение",
    "- при `AGENT_STATUS: blocked_by_invariant` сразу после статуса добавь однострочные поля `VIOLATED_INVARIANT: ...`, `REASON: ...`, `USER_MESSAGE: ...`",
...
function renderMemoryControls() {
  ...
  invariantsStatusElement.textContent = formatMemoryStatus(invariants);
...
function getMemoryDialogCopy(kind: MemoryKind) {
  if (kind === "invariants") {
    return {
      eyebrow: "Инварианты",
      title: "Инварианты ассистента",
      label: "Правила, которые нельзя нарушать",
      description:
        "Эти инварианты хранятся в invariants.json, добавляются в системный промпт перед памятью и имеют приоритет над запросом пользователя.",
    };
  }
...
async function hydrateSavedChats() {
  try {
    const [loadedSavedChats, loadedProjectMemory, loadedLongTermMemory, loadedInvariants, loadedUserProfile] =
      await Promise.all([
        loadSavedChats(),
        loadProjectMemory(),
        loadLongTermMemory(),
...
async function selectStorageDirectory() {
  try {
    ...
    const [loadedProjectMemory, loadedLongTermMemory, loadedInvariants, loadedUserProfile] = await Promise.all([
      loadProjectMemory(),
      loadLongTermMemory(),
      loadInvariants(),
      loadUserProfile(),
...
async function saveActiveMemoryDocument() {
  ...
  } else if (activeMemoryKind === "invariants") {
    invariants = await persistInvariants(content);
  } else {
...
function createLifecycleBlockedTaskState(taskState: TaskState, reason: string): TaskState {
  return {
    ...taskState,
    isPaused: true,
    expectedAction: `Разобрать блокировку жизненного цикла: ${reason}`,
  };
}
...
function shouldRunInvariantPreflight(taskState: TaskState) {
  return (
    !!invariants.content.trim() &&
    taskState.phase === "research" &&
    !taskState.isPaused &&
    taskState.steps.every((step) => step.status === "pending" && !step.agentDialogId)
  );
}

function createInvariantPreflightPrompt(parentTitle: string, parentTaskBrief: string) {
  return [
    "Проверь, можно ли выполнять задачу без нарушения инвариантов.",
    "",
    "Инварианты:",
    invariants.content.trim(),
    "",
    `Название задачи: ${parentTitle}`,
    "",
    "Запрос пользователя:",
    parentTaskBrief,
    "",
    "Ответь строго в таком формате:",
    "PREFLIGHT_STATUS: allowed | blocked_by_invariant",
    "VIOLATED_INVARIANT: <если blocked_by_invariant, укажи конкретный инвариант; иначе пусто>",
    "REASON: <краткое объяснение>",
    "USER_MESSAGE: <сообщение пользователю, если выполнение невозможно; иначе пусто>",
    "",
    "Если запрос можно выполнить допустимой альтернативой без нарушения инвариантов, верни allowed.",
    "Если запрос требует нарушить хотя бы один инвариант, верни blocked_by_invariant.",
  ].join("\n");
}

function parseInvariantPreflightResult(content: string): AgentStructuredStatus {
  const status = parseStructuredField(content, "PREFLIGHT_STATUS").toLowerCase();
  const isBlocked = status.includes("blocked_by_invariant") || hasInvariantConflictSignal(content);

  return {
    status: isBlocked ? "blocked_by_invariant" : "done",
    details: {
      violatedInvariant: parseStructuredField(content, "VIOLATED_INVARIANT"),
      reason: parseStructuredField(content, "REASON"),
      userMessage: parseStructuredField(content, "USER_MESSAGE"),
      summary: content.trim(),
    },
  };
}

function parseAgentStructuredStatus(content: string): AgentStructuredStatus {
  const status = parseStructuredField(content, "AGENT_STATUS").toLowerCase();
  const normalizedStatus = status.includes("blocked_by_invariant")
    ? "blocked_by_invariant"
    : status.includes("blocked")
      ? "blocked"
      : status.includes("failed")
        ? "failed"
        : hasInvariantConflictSignal(content)
          ? "blocked_by_invariant"
          : "done";
...
function parseStructuredField(content: string, fieldName: string) {
  const pattern = new RegExp(`^${fieldName}:\\s*(.*)$`, "im");
  return content.match(pattern)?.[1]?.trim() || "";
}

function hasInvariantConflictSignal(content: string) {
  const text = content.toLowerCase();
  return (
    (text.includes("инвариант") || text.includes("invariant")) &&
    (text.includes("наруш") || text.includes("конфликт") || text.includes("violate") || text.includes("conflict")) &&
    (text.includes("не могу") || text.includes("нельзя") || text.includes("отказ") || text.includes("cannot") || text.includes("refuse"))
  );
}

function createInvariantBlockedMessage(details: InvariantBlockDetails) {
  const violatedInvariant = details.violatedInvariant.trim();
  const reason = details.reason.trim();
  const userMessage = details.userMessage.trim();

  return [
    "Выполнение невозможно: запрос нарушает инварианты.",
    violatedInvariant ? `Инвариант: ${violatedInvariant}` : "",
    reason ? `Причина: ${reason}` : "",
    userMessage || (!reason && !violatedInvariant ? details.summary : ""),
  ]
    .filter(Boolean)
    .join("\n\n");
}

async function blockSavedTaskByInvariant(
  parentChat: SavedChat,
  taskState: TaskState,
  step: TaskStep | null,
  details: InvariantBlockDetails,
  options: {
    usage?: TokenUsage;
    agentDialogId?: string;
    artifactPaths?: string[];
  } = {},
) {
  const now = new Date().toISOString();
  const blockedMessage = createInvariantBlockedMessage(details);
  const nextSteps = taskState.steps.map((item) =>
    item.id === (step?.id || taskState.currentStepId)
      ? {
          ...item,
          status: "blocked" as TaskStepStatus,
          artifactPaths: options.artifactPaths || item.artifactPaths || [],
          resultSummary: blockedMessage,
          agentDialogId: options.agentDialogId || item.agentDialogId,
          updatedAt: now,
        }
      : item,
  );
  const nextParentChat = addSavedChatTokenUsage(
    {
      ...parentChat,
      messages: [
        ...parentChat.messages,
      ...
      taskState: touchTaskState({
        ...taskState,
        currentStepId: step?.id || taskState.currentStepId,
        isPaused: true,
        expectedAction: "Выполнение невозможно: запрос нарушает инварианты.",
        steps: nextSteps,
      }),
      updatedAt: now,
    },
  ...
  return persistAndReplaceSavedChat(nextParentChat, parentChat.fileName);
}

async function runInvariantPreflight(parentChat: SavedChat, taskState: TaskState) {
  if (!shouldRunInvariantPreflight(taskState)) {
    return parentChat;
  }

  const prompt = createInvariantPreflightPrompt(
    parentChat.title,
    getParentTaskBriefFromMessages(parentChat.messages, parentChat.title),
  );
  const preflightSettings: ChatSettings = {
    ...createSettingsForSavedChat(parentChat),
    systemPrompt: "Ты проверяешь только конфликт задачи с инвариантами. Не решай задачу.",
    temperature: 0,
    maxTokens: 600,
    responseFormat: "text",
    thinkingMode: "disabled",
  };
  const result = await requestChatCompletion([{ role: "user", content: prompt }], preflightSettings);
  await appendSavedChatLog(parentChat, [createExchangeLogEntry(result.exchangeLog)]).catch(() => undefined);

  const parsedResult = parseInvariantPreflightResult(result.content);

  if (parsedResult.status !== "blocked_by_invariant") {
    return addSavedChatTokenUsage(parentChat, result.usage);
  }

  return blockSavedTaskByInvariant(parentChat, taskState, getCurrentStep(taskState), parsedResult.details, {
    usage: result.usage,
  });
}
...
async function runSavedOrchestratorWorkflow(chatId: string) {
  ...
  while (true) {
    let taskState = normalizeTaskState(parentChat.taskState || createInitialTaskState());

    parentChat = await runInvariantPreflight(parentChat, taskState);
    taskState = normalizeTaskState(parentChat.taskState || createInitialTaskState());
    ...
    const runCheck = canRunCurrentStep(taskState);

    if (!runCheck.ok) {
      await pauseSavedTaskByLifecycle(parentChat, taskState, runCheck.reason);
      return;
    }
    ...
    const transition = advanceTaskPhase(completedTaskState);

    if (!transition.ok) {
      await pauseSavedTaskByLifecycle(parentChat, completedTaskState, transition.reason);
      return;
    }
    ...
    parentChat = await persistAndReplaceSavedChat(
      {
        ...
        taskState: touchTaskState(transition.taskState),
        updatedAt: new Date().toISOString(),
      },
      parentChat.fileName,
    );
...
async function pauseSavedTaskByLifecycle(parentChat: SavedChat, taskState: TaskState, reason: string) {
  const now = new Date().toISOString();
  ...
  return persistAndReplaceSavedChat(
    {
      ...
      messages: [
        ...
        {
          role: "assistant",
          content: createInvalidTransitionAssistantMessage(reason),
          createdAt: now,
        },
      ],
      taskState: touchTaskState(createLifecycleBlockedTaskState(taskState, reason)),
      updatedAt: now,
    },
    parentChat.fileName,
  );
...
async function runSavedStepAgent(parentChat: SavedChat, step: TaskStep) {
  ...
  const agentStatus = parseAgentStructuredStatus(result.content);
  const currentState = normalizeTaskState(parentChat.taskState || createInitialTaskState());

  if (agentStatus.status === "blocked_by_invariant") {
    const nextParentChat = await blockSavedTaskByInvariant(parentChat, currentState, activeStep, agentStatus.details, {
      usage,
  ...
  if (agentStatus.status === "blocked" || agentStatus.status === "failed") {
    const summary = result.content.trim() || "Агент не смог завершить шаг.";
    const status = agentStatus.status === "failed" ? "failed" : "blocked";
    const nextParentChat = await persistAndReplaceSavedChat(
      addSavedChatTokenUsage(
        {
          ...parentChat,
          messages: [
            ...
            {
              role: "assistant",
              content: `Агент этапа "${activeStep.title}" не смог завершить шаг.\n\n${summary}`,
              createdAt: new Date().toISOString(),
            },
          ],
          taskState: touchTaskState({
            ...
            isPaused: true,
            expectedAction: `Разобрать блокировку агента этапа "${activeStep.title}".`,
...
appRoot.addEventListener("click", (event) => {
  ...
  if (button.dataset.action === "open-invariants") {
    openMemoryDialog("invariants");
    return;
  }
...
composer.addEventListener("submit", async (event) => {
  ...
  if (currentAgentMetadata.role === "orchestrator") {
    ...
    if (userApprovedStage) {
      const transition = advanceTaskPhase(taskState);

      if (!transition.ok) {
        setCurrentTaskState(createLifecycleBlockedTaskState(taskState, transition.reason));
        messages = [
          ...messages,
          {
            role: "assistant",
            content: createInvalidTransitionAssistantMessage(transition.reason),
            createdAt: new Date().toISOString(),
          },
        ];
        await saveActiveTaskState();
      ...
      setCurrentTaskState(transition.taskState);
      messages = [
        ...messages,
        {
          role: "assistant",
