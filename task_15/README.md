...
В агентном режиме задача управляется явным конечным автоматом:

```text
research -> execution -> validation -> done
```

Оркестратор не может перейти к следующему этапу, пока текущий этап не завершен со статусом `done`. Перед переходом запускается программная проверка lifecycle: нельзя начать `execution`, пока не завершен `research`; нельзя перейти к `validation`, пока не завершен `execution`; нельзя поставить задачу в `done`, пока не завершен этап `validation`.

Если загруженное состояние диалога повреждено или будущая логика попытается перепрыгнуть этап, workflow останавливается: задача переводится на паузу, а в главный диалог добавляется сообщение `Переход остановлен: нарушен жизненный цикл задачи.` с причиной блокировки. После паузы пользователь может исправить ситуацию или продолжить с ожидаемого действия, и оркестратор возобновит работу с сохраненного этапа.
