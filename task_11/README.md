...
В шаблоне есть три типа памяти, и они хранятся отдельно:
...
Память проекта и долговременная память редактируются вручную из правой панели кнопками `Память проекта` и `Долговременная память`. Сохранение каждой кнопки пишет только свой файл. Если файла еще нет, пустая память считается нормальным состоянием.

При отправке нового запроса шаблон собирает итоговый системный промпт так:

1. системный промпт из настроек;
2. блок `Долговременная память`;
3. блок `Память проекта`.
