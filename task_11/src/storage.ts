import type { AiExchangeLogEntry, MemoryDocument, SavedChat, SavedChatMetadata, TokenUsage, UiChatMessage } from "./types";
...
async function readSavedChatsFromDirectory(
  ...
  for await (const entry of toFileStorageDirectory(directory).values()) {
    ...
    if (entry.name === projectMemoryFileName || entry.name === longTermMemoryFileName) {
      continue;
    }
