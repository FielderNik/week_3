import "./styles.css";
import { createMemoryAwareSettings, emptyMemoryDocument } from "./memory";
...
async function hydrateSavedChats() {
  try {
    const [loadedSavedChats, loadedProjectMemory, loadedLongTermMemory] = await Promise.all([
      loadSavedChats(),
      loadProjectMemory(),
      loadLongTermMemory(),
    ]);
...
async function selectStorageDirectory() {
  try {
    ...
    const [loadedProjectMemory, loadedLongTermMemory] = await Promise.all([loadProjectMemory(), loadLongTermMemory()]);
...
composer.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const memoryAwareSettings = createMemoryAwareSettings(settings, longTermMemory, projectMemory);
